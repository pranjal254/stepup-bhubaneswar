...
export default function RegisterPage() {
  ...
  const getPricing = (selectedSongs = []) => {
    if (selectedSongs.length === 0) return 0;
    
    // Special combo price if all 3 songs are selected
    if (selectedSongs.length === 3) {
      return 3000;
    }
    
    // Calculate total based on selected songs
    return selectedSongs.reduce((total, songId) => {
      const song = songOptions.find(s => s.id === songId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/register", {
        ...
        body: JSON.stringify({
          ...formData,
          songs: formData.selectedSongs.length, // Calculate songs count from selection
        })
      });
                ...
                <div className="border-t pt-4">
                  ...
                  {formData.selectedSongs.length > 0 && (
                    <div className="mt-2 text-xs text-green-600">
                      {formData.selectedSongs.length} song{formData.selectedSongs.length !== 1 ? 's' : ''} selected
                    </div>
                  )}
                </div>
                ...
                <div className="border-t pt-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-2xl font-bold text-orange-500">
                      ₹{getPricing(formData.selectedSongs)}
                    </span>
                  </div>
                  {formData.selectedSongs.length === 0 && (
                    <div className="text-xs text-gray-500">
                      Select songs to see total
                    </div>
                  )}
                  {formData.selectedSongs.length === 3 && (
                    <div className="text-xs text-green-600 font-medium">
                      You save ₹200 with the combo!
                    </div>
                  )}
                </div>
              </div>
